...
export const duckDuckGoKeywords = tool(
  async (input) => {
    // Handle both string input and object input with proper validation
    let query, maxSuggestions;
    ...
    if (typeof input === "string") {
      ...
      throw new Error(
        "Invalid input: expected string or object with query property"
      );
    }
    ...
    try {
      ...
      const instantAnswersUrl = `https://api.duckduckgo.com/?q=${encodeURIComponent(
        query
      ...
      if (instantData.RelatedTopics && instantData.RelatedTopics.length > 0) {
        results.relatedTopics = instantData.RelatedTopics.slice(
          0,
          maxSuggestions
        )
          .map((topic) => {
      ...
      const autocompleteUrl = `https://duckduckgo.com/ac/?q=${encodeURIComponent(
        query
      )}&type=list`;
  ...
  },
  {
    ...
    description:
      "Fetch related topics, autocomplete suggestions, and generate keywords using DuckDuckGo free APIs",
    parameters: {
      type: "object",
      properties: {
        query: {
          type: "string",
          description:
            "The search query to get related topics and suggestions for",
        },
        maxSuggestions: {
          type: "number",
